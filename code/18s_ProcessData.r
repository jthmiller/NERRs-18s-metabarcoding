...
pcrux <- qza_to_phyloseq(
  tree="results/NERRS_18s_9_12_24_rooted-tree.qza", 
  features='results/NERRS_18s_euks_hum_samples-table.qza', 
  taxonomy='results/NERRS_18s_vsearch_taxonomy_10accepts_90perc_CRUX.qza',
  metadata='metadata/phylo-metadata.tsv'
  )
...
ps_fam <- tax_glom(psilva, "Family")
pcphy <- tax_glom(pcrux, "Phylum")
pc_Cla <- tax_glom(pcrux, "Phylum")

tax_table(ps_species)[,'Species']

ps_species <- tax_glom(psilva, "Species")
pc_species <- tax_glom(pcrux, "Species")
